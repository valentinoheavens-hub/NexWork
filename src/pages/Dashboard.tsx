...
const Dashboard = () => {
  ...
  const quickActions = [
    { name: "New Project", icon: Target, href: "/project/new", color: "bg-emerald-50 text-emerald-600" },
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <Link to="/invoice/new">
              <Button className="bg-emerald-600 hover:bg-emerald-700 text-white">Quick Invoice</Button>
            </Link>
          </div>
        </div>
            ...
            <Link key={action.name} to={action.href}>
              <Card className="border-none shadow-sm hover:shadow-md transition-all group cursor-pointer">
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <span className="font-bold text-sm text-slate-700 group-hover:text-emerald-600 transition-colors">{action.name}</span>
                </CardContent>
              </Card>
            </Link>
          ))}
